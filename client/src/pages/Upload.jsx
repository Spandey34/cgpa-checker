...
export default function Upload() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setResult(data);
      toast.success(
        `Processed ${data.totalRows} rows — ${data.flaggedCount} flagged`,
      );
    } catch (err) {
    ...
    <div className="p-8 max-w-4xl mx-auto">
      <div className="mb-8 animate-fade-up">
        ...
        <p className="text-slate-500 text-sm mt-1">
          Upload a Google Form response sheet to flag discrepancies.
        </p>
      </div>
      ...
      {!result ? (
        <div className="space-y-5 animate-fade-up-delay-1">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-slate-500 text-xs">
                Rows where |entered CGPA − actual CGPA| &gt;{" "}
                {threshold || "0.02"} will be flagged red
              </span>
            </div>
          </div>
              ...
              {file ? (
                ...
                <div>
                  ...
                  <p className="text-slate-600 text-xs mt-2">
                    ...
                    and <span className="font-mono text-slate-500">CGPA</span>
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="bg-slate-800/30 border border-slate-700/40 rounded-lg px-4 py-3 text-xs text-slate-500">
            ...
            <p>
              CGPA:{" "}
              <span className="font-mono">
                CGPA, GPA, Entered CGPA, Self CGPA
              </span>
            </p>
          </div>
        ...
        <div className="animate-fade-up">
          <ResultPanel
            ...
            onReset={reset}
            onViewHistory={() => navigate(`/history/${result.historyId}`)}
          />
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <SummaryCard
          label="Clean"
          value={result.totalRows - result.flaggedCount - result.notFoundCount}
      ...
      <div>
              ...
              <thead className="sticky top-0">
                <tr className="bg-slate-900 border-b border-slate-800">
                  {[
                    ...
                    <th
                      key={h}
                      className="px-4 py-3 text-left text-slate-400 font-mono font-medium whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {result.results?.map((row, i) => (
                  ...
                  >
                    ...
                    <td
                      className={`px-4 py-2.5 font-mono font-bold ${row.flagged ? "text-red-400" : "text-slate-400"}`}
                    >
                      {row.difference !== null
        ...
        {result.results?.length >= 100 && (
          <p className="text-slate-600 text-xs mt-2 text-center">
            Showing first 100 rows · download Excel for full data
          </p>
        )}
      </div>
    </div>
...
function StatusBadge({ flagged, found }) {
  if (!found)
    return (
      <span className="px-2 py-0.5 rounded bg-amber-500/15 text-amber-400 font-mono">
        NOT FOUND
      </span>
    );
  if (flagged)
    return (
      <span className="px-2 py-0.5 rounded bg-red-500/15 text-red-400 font-mono">
        FLAGGED
      </span>
    );
  return (
    <span className="px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-400 font-mono">
      OK
    </span>
  );
}
